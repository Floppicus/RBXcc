  ...
  function Rcon(host, port, password, options) {
    ...
    _this.send = function (data, cmd, id) {
      ...
      if (_this.tcp) {
        ...
        sendBuf = buffer.Buffer.alloc(length + 14);
        ...
        str += data + '\n';
        sendBuf = buffer.Buffer.alloc(4 + buffer.Buffer.byteLength(str));
    ...
    _this.connect = function () {
      if (_this.tcp) {
        _this._tcpSocket = net.createConnection(_this.port, _this.host);
        ...
        _this._udpSocket = dgram.createSocket('udp4');
    ...
    _this._tcpSocketOnData = function (data) {
      if (_this.outstandingData != null) {
        data = buffer.Buffer.concat([_this.outstandingData, data], _this.outstandingData.length + data.length);
    ...
    _this.socketOnConnect = function () {
      ...
      if (_this.tcp) {
        ...
        var sendBuf = buffer.Buffer.alloc(str.length + 4);
        sendBuf.writeInt32LE(-1, 0);
        sendBuf.write(str, 4);

        _this._sendSocket(sendBuf);
      } else {
        var _sendBuf = buffer.Buffer.alloc(5);
    ...
    events.EventEmitter.call(_assertThisInitialized(_this));
    return _this;
  }
...
//# sourceMappingURL=ts-rcon.cjs.development.js.map
